...
export function NuovoAccount() {
    ...
    const aggiunta = useCallback(async () => {
        let res = await fetch("/Login/Account", { body: JSON.stringify({ username, password, am }), method: "POST", headers: { 'Content-type': 'application/json; charl set=UTF-8' } });
        if (res.status === 200) {
            setIstrue(true);
        }
        ...
        <div className="row container">
            <form className="aa">
                ...
                <div className=" form-floating  md-3 UserNametex has-validation ">
                    <input type="text" value={username} className={"form-control "} placeholder=" " onChange={(a) => { setUsername(a.target.value); }} />
